...
Rendering and export processing fail closed unless all of these process settings
are present together: `AI_LECTURE_DEPLOYMENT_MODE=local-evaluation`,
`AI_LECTURE_LOCAL_EXECUTION_HOST` is `localhost` or a numeric loopback address,
`AI_LECTURE_VIDEO_RENDER_ADAPTER=1`, and the exact evaluation acknowledgement is
set. Any Railway environment disables processing regardless of those values.
Production, staging, operational, published, commercial, disabled, empty, and
unknown deployment modes are rejected. Railway remains suitable for UI and
workflow staging, but never for Remotion render or export workers.

For an authorized local evaluation process only:
...
Failed renders are never reset or overwritten. An administrator may submit a
CSRF-protected recovery request with a reason; the service revalidates the exact
current approvals, immutable input snapshot, selected takes, recordings and
hashes, then creates a separately addressed render version and immutable recovery
audit record. Only the latest failed version is eligible, and each failed version
can be recovered at most once.
