...
## D017 — Extractive source-grounding and provider-independent approval domain

- **Status:** Accepted for T022 review
- **Decision:** Use a deterministic local extractive generator for Phase 1. Snapshot the exact authorized extraction version and reviewed page text, require every factual slide claim and narration statement to reference an exact snapshot range, keep revisions append-only, invalidate approval on teacher edit, and expose captions only from the currently approved narration revision. Generation receives an internal actor authorization context rather than a Django/CRM session, and a provider protocol permits a later separately authorized generator without changing provenance or approval records. CRM/SSO integration remains deferred.
- **Reason:** Exact extractive grounding provides reproducible, zero-cost synthetic verification and fails closed rather than inventing facts. Immutable snapshots preserve historical citations, while an authentication-provider adapter boundary and stable review model avoid coupling future identity or generation-provider choices to teacher approval.

## D018 — Database-backed workflow queue and non-bypassable approval state machine

- **Status:** Accepted for T030 review
- **Decision:** Use a PostgreSQL-portable Django database queue with explicit bounded coordination job types, transactional claim/lease/retry/idempotency behavior and immutable workflow/job audit events. Keep SQLite strictly sequential and single-worker for the Phase-1 pilot. Require PostgreSQL plus a controlled worker deployment before concurrent production. Authorize the state machine through provider-independent teacher, administrator and system-worker contexts; preserve separate teacher slide/narration and video approvals plus final administrator approval, and invalidate downstream state when a current slide revision changes.
- **Reason:** Phase 1 needs durable local coordination and approval evidence without Redis, Celery, cloud queues, paid services or CRM coupling. Explicit state/current-version validation, successful-job evidence and actor-specific gates prevent skipped work, stale approvals, role substitution and silent advancement after failure, while the PostgreSQL locking path provides the required foundation for a later controlled concurrent deployment.

## D019 — Evaluation-only local Remotion composition and immutable video versions

- **Status:** Accepted for T050 review
- **Decision:** Use exact official Remotion `4.0.514` packages as the primary local composition engine only during the current non-commercial evaluation. Use Remotion-managed Chrome Headless Shell and FFmpeg through a fixed server-side adapter. Snapshot exact T022/T040 inputs, create append-only draft and export versions, require transcript-backed teacher approval for spoken-content removal, and satisfy existing T030 draft/export jobs before readiness transitions. Keep HTTP worker operations fail-closed and stop before production or commercial use until licence eligibility is reassessed.
- **Reason:** Deterministic local compositions support bilingual branded lectures without transferring private media. Fixed commands, project-contained ignored storage, immutable evidence and separate teacher/admin gates prevent injection, stale approval reuse, silent semantic cuts and historical overwrite. The Remotion free licence covers evaluation before commercial use but does not establish ongoing institutional production eligibility.
