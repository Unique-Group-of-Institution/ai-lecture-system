...
`data/exports/t050-video` together while stopped. A failure retains raw inputs and
older versions. Export staging is operation-owned and removed on failure; final
directories are renamed into place only after every file and hash exists. Never
promote a `.part` file manually. Request a new version after recovery. Removing
either process-only adapter flag disables worker authority without changing
stored evidence. No cleanup command deletes source recordings or teacher content.
