...
class RemotionVideoAssemblyTests(TestCase):
    ...
    def test_failed_initial_render_recovery_is_admin_only_audited_and_immutable(self):
        failed = request_initial_render(actor=self.admin_actor, workflow_id=self.workflow.pk)
        with patch("lectures.video._invoke_adapter", side_effect=RuntimeError("synthetic")):
        ...
        with self.assertRaises(PermissionDenied):
            recover_failed_render(
                actor=self.teacher_actor, workflow_id=self.workflow.pk,
                failed_render_id=failed.pk, reason="Synthetic retry",
            )
        replacement = recover_failed_render(
            actor=self.admin_actor, workflow_id=self.workflow.pk,
            failed_render_id=failed.pk, reason="Synthetic local failure recovery",
        )
        failed.refresh_from_db()
        self.assertEqual(failed.status, VideoRenderVersion.Status.FAILED)
        self.assertEqual((replacement.parent_id, replacement.render_input_id), (failed.pk, failed.render_input_id))
        self.assertNotEqual(replacement.video_storage_key, failed.video_storage_key)
        self.assertEqual((self.video_root / failed.manifest_storage_key).read_bytes(), old_manifest)
        recovery = VideoRenderRecovery.objects.get(failed_render=failed)
        self.assertEqual(recovery.replacement_render, replacement)
        with self.assertRaises(VideoConflict):
            recover_failed_render(
                actor=self.admin_actor, workflow_id=self.workflow.pk,
                failed_render_id=failed.pk, reason="Duplicate",
            )

    def test_failed_derived_recovery_preserves_approved_edit_chain(self):
        first = self.rendered()
        decision, failed = create_edit_decision(
            actor=self.admin_actor, workflow_id=self.workflow.pk, base_render_id=first.pk,
        ...
        replacement = recover_failed_render(
            actor=self.admin_actor, workflow_id=self.workflow.pk,
            failed_render_id=failed.pk, reason="Recover derived render",
        )
        self.assertEqual(replacement.parent_id, failed.pk)
        self.assertEqual(
            list(replacement.applied_edits.values_list("decision_id", flat=True)), [decision.pk]
        )

    def test_recovery_http_requires_csrf_and_administrator(self):
        failed = request_initial_render(actor=self.admin_actor, workflow_id=self.workflow.pk)
        with patch("lectures.video._invoke_adapter", side_effect=RuntimeError("synthetic")):
            with self.assertRaises(RuntimeError):
                process_render(failed.pk)
        url = f"/api/video/workflows/{self.workflow.pk}/renders/{failed.pk}/recover/"
        teacher_client = Client(enforce_csrf_checks=True)
        teacher_client.force_login(self.teacher)
        self.assertEqual(
            teacher_client.post(url, data=json.dumps({"reason": "No"}), content_type="application/json").status_code,
            403,
        )
        admin_client = Client(enforce_csrf_checks=True)
        admin_client.force_login(self.admin)
        self.assertEqual(
            admin_client.post(url, data=json.dumps({"reason": "Retry"}), content_type="application/json").status_code,
            403,
        )
        token = "a" * 32
        admin_client.cookies["csrftoken"] = token
        response = admin_client.post(
            url, data=json.dumps({"reason": "Audited recovery"}),
            content_type="application/json", HTTP_X_CSRFTOKEN=token,
        )
        self.assertEqual(response.status_code, 201)

    def test_local_evaluation_boundary_fails_closed_for_all_other_modes_and_hosts(self):
        for mode in ("production", "railway", "staging", "operational", "published", "commercial", "disabled", "unknown", ""):
            render = request_initial_render(actor=self.admin_actor, workflow_id=self.workflow.pk)
            with self.subTest(mode=mode), override_settings(VIDEO_RENDER_DEPLOYMENT_MODE=mode), self.assertRaises(UnsupportedRenderEnvironment):
                process_render(render.pk)
            render.delete = lambda *args, **kwargs: None
            VideoRenderVersion.objects.filter(pk=render.pk).delete()
        render = request_initial_render(actor=self.admin_actor, workflow_id=self.workflow.pk)
        for host in ("0.0.0.0", "example.test", "192.168.1.5", ""):
            with self.subTest(host=host), override_settings(VIDEO_RENDER_EXECUTION_HOST=host), self.assertRaises(UnsupportedRenderEnvironment):
                process_render(render.pk)
        with override_settings(VIDEO_RENDER_RAILWAY_ENVIRONMENT=True), self.assertRaises(UnsupportedRenderEnvironment):
            process_render(render.pk)
    ...
    def test_multi_slide_timeline_is_consecutive_and_srt_uses_exact_render_frames(self):
        first = self.rendered()
        narration = first.render_input.items.get(position=1)
        decision, pending_cut_render = create_edit_decision(
            actor=self.admin_actor,
            workflow_id=self.workflow.pk,
            base_render_id=first.pk,
            ...
            payload={
                ...
                "reason": "Synthetic frame-timeline regression",
            },
        )
        self.assertIsNone(pending_cut_render)
        cut_render = approve_spoken_edit(actor=self.teacher_actor, decision_id=decision.pk)
        with patch("lectures.video._invoke_adapter", side_effect=self.fake_render):
            process_render(cut_render.pk)
        _, timed_render = create_edit_decision(
            actor=self.admin_actor,
            workflow_id=self.workflow.pk,
            base_render_id=cut_render.pk,
            decision_type=VideoEditDecision.DecisionType.TRANSITION_TIMING,
            payload={"transition_ms": 300, "hold_after_ms": 100},
        )
        with patch("lectures.video._invoke_adapter", side_effect=self.fake_render):
            process_render(timed_render.pk)

        manifest = json.loads(
            (self.video_root / timed_render.manifest_storage_key).read_text(encoding="utf-8")
        )
        self.assertEqual(manifest["transitionMs"], 300)
        self.assertEqual(
            [
                (
                    slide["timelineStartFrame"],
                    slide["narrationFrames"],
        ...
        self.assertEqual(manifest["totalDurationInFrames"], 87)
        self.assertEqual(
            _build_srt(manifest),
            "1\n00:00:00,000 --> 00:00:01,300\n"
            f"{manifest['slides'][0]['caption']}\n\n"
            "2\n00:00:01,300 --> 00:00:02,900\n"
            f"{manifest['slides'][1]['caption']}\n",
        )

    def test_video_detail_reports_all_unsupported_render_environments(self):
        client = Client()
        client.force_login(self.admin)
        url = f"/video/workflows/{self.workflow.pk}/"
        unsupported = (
            {"VIDEO_RENDER_DEPLOYMENT_MODE": "production"},
            {"VIDEO_RENDER_RAILWAY_ENVIRONMENT": True},
            {"VIDEO_RENDER_EXECUTION_HOST": "railway.internal"},
        )
        for settings_override in unsupported:
            with self.subTest(settings=settings_override), override_settings(
                **settings_override
            ):
                response = client.get(url)
                self.assertEqual(response.status_code, 200)
                self.assertContains(response, "Unsupported environment")
                self.assertContains(response, "Request draft render", html=False)
    ...
    def test_review_and_export_gates_use_exact_t030_jobs_and_approval_order(self):
        ...
        with override_settings(VIDEO_RENDER_DEPLOYMENT_MODE="production"), self.assertRaises(
            UnsupportedRenderEnvironment
        ):
            process_next_readiness_job()
        package.refresh_from_db()
        self.assertEqual(package.status, VideoExportPackage.Status.PENDING)
        process_next_readiness_job()
        package.refresh_from_db()
        self.workflow.refresh_from_db()
