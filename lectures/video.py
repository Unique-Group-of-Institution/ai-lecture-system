...
def _input_is_current(render_input: VideoRenderInput, workflow: LectureWorkflow) -> None:
    ...
    items = list(render_input.items.select_related("take").order_by("position"))
...
def _trusted_worker(workflow: LectureWorkflow) -> WorkflowActorContext:
    if not settings.VIDEO_RENDER_ADAPTER_ENABLED or not settings.VIDEO_RENDER_EVALUATION_ACK:
...
def _build_srt(manifest: dict) -> str:
    current = 0
    blocks = []
    hold = manifest["holdAfterMs"]
    for index, slide in enumerate(manifest["slides"], 1):
        removed = sum(cut["endMs"] - cut["startMs"] for cut in slide["cuts"])
        duration = slide["durationMs"] - removed + hold
        blocks.append(
