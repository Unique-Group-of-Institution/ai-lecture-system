...
const msToFrames = (milliseconds, fps) => Math.max(0, Math.round((milliseconds / 1000) * fps));
...
exact(manifest, ['schemaVersion','compositionId','fps','width','height','renderReference','publicDir','slides','branding','captions','transitionMs','holdAfterMs','totalDurationInFrames'], 'manifest');
...
integer(manifest.totalDurationInFrames, 1, 3600000, 'total duration');
...
for (const [index, slide] of manifest.slides.entries()) {
  exact(slide, ['position','title','claims','caption','audioSrc','durationMs','cuts','timelineStartFrame','narrationFrames','durationInFrames'], 'slide');
  ...
  integer(slide.timelineStartFrame, 0, 3600000, 'slide timeline start');
  integer(slide.narrationFrames, 0, 3600000, 'slide narration duration');
  integer(slide.durationInFrames, slide.narrationFrames, 3600000, 'slide duration');
  if (slide.timelineStartFrame !== timelineCursor) throw new Error('Slides must have a consecutive timeline.');
  timelineCursor += slide.durationInFrames;
  if (!Array.isArray(slide.cuts) || slide.cuts.length > 10) throw new Error('Invalid spoken-content edits.');
  let cursor = 0;
  let expectedNarrationFrames = 0;
  for (const cut of slide.cuts) {
    ...
    if (cut.startMs > cursor) expectedNarrationFrames += Math.max(1, msToFrames(cut.startMs - cursor, manifest.fps));
    cursor = cut.endMs;
  }
  if (cursor < slide.durationMs) expectedNarrationFrames += Math.max(1, msToFrames(slide.durationMs - cursor, manifest.fps));
  const expectedDuration = Math.max(1, expectedNarrationFrames + msToFrames(manifest.holdAfterMs, manifest.fps));
  if (slide.narrationFrames !== expectedNarrationFrames || slide.durationInFrames !== expectedDuration) {
    throw new Error('Slide timeline does not match its audio edits and hold.');
  }
}
